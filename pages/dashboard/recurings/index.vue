                ...
                <div class="card shadow-lg mb-5 bg-white rounded">
                    <div class="card-body">
                        <div class="float-left">
                            <h2 class="card-title"> Recuring </h2>
                            <h3 class="card-subtitle mb-2">{{ total }}</h3>
                        </div>
                        <div class="float-right">
                            <b-icon font-scale="3.3" icon="arrow-repeat" variant="primary" aria-hidden="true"></b-icon>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <Table>
                            ...
                            <tbody v-if="transactions.length != 0">
                                <tr v-for="(transaction, index) in transactions" :key="index">
                                    <th>{{ index+1 }}</th>
                                    <td>
                                        <b-link :to="`/dashboard/users/${transaction.user.id}`">
                                    ...
                                    <td>
                                        <b-button  size="sm" :to="`/dashboard/recurings/${transaction.id}`" variant="primary">More</b-button>
                                    </td>
                                </tr>
                            </tbody>
                            <tbody v-else>
...
export default {
    ...
    methods: {
        ...
        getransactions() {
            ...
            this.$axios.post(`${process.env.baseUrl}recuring/all`,
                {
                ...
                this.transactions = res.data.recuring.data
                this.first_page_url = res.data.recuring.first_page_url
                this.last_page_url = res.data.recuring.last_page_url
                this.next_page_url = res.data.recuring.next_page_url
                this.prev_page_url = res.data.recuring.prev_page_url
                this.to = res.data.recuring.to
                this.total = res.data.recuring.total
                this.from = res.data.recuring.from

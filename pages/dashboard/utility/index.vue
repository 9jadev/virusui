<template>
    <div class="container m-2">
        <h1 class="my-4">Utility bills</h1>
                        ...
                        <Table>
                            ...
                            <tbody v-if="kycs.length != 0 && kycs != null">
                                <tr v-for="(transaction, index) in kycs" :key="index">
                                    ...
                                    <td>  <b-link target="_blank" :href="transaction.bill_image">View</b-link> </td>
                                    <td>
                                        <b-badge v-if="transaction.verify_utility == 1" variant="success"> Verified </b-badge>
                                        <b-badge v-else variant="danger">Not Verified </b-badge>
                                    </td>
                                    <td>
    ...
    methods: {
        money_format(number) {
         ...
         unconfirmation(ids) {
            ...
            console.log(user)
            swal("UnConfimation notice", `You are about to reject ${this.kycs[iden].user.firstname} ${this.kycs[iden].user.lastname} Utility bill.`, "info").then((re) => {
                if (re == true) {
                    this.tableloading = true
                    this.$axios.post(`${process.env.baseUrl}kycverification/admin/unconfirm/utility`, {"id": user.id}).then((res) => {
        ...
        confirmation(ids){
            ...
            let user = this.kycs[iden]
            console.log(user)
            swal("Confimation notice", `You are about to confirm ${this.kycs[iden].user.firstname} ${this.kycs[iden].user.lastname} Utility Bill.`, "info").then((re) => {
                if (re == true) {
                    this.tableloading = true
                    this.$axios.post(`${process.env.baseUrl}kycverification/admin/retrive/user/utility`, {"id": user.id}).then((res) => {
                        if (res.data.status == "success") {
                            window.location.reload()
                        }
                        this.tableloading = false
        ...
        getransactions() {
            this.tableloading = true
            this.$axios.post(`${process.env.baseUrl}kycverification/admin/retrive/utility`,
